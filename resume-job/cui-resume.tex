...
\hspace{-1em} Programming languages & Python, C$+$$+$, C, Java, CUDA\\
\hspace{-1em} Machine learning softwares & TensorFlow, Caffe, Spark, Matlab, PyTorch, GraphLab\\
\hspace{-1em} Big data softwares & MPI, TBB, HBase, OpenTSDB, Hadoop\\
\end{tabular}
\end{rSection}
\vspace{-.05in}
...
\vspace{-0.3em}
\item[] \hspace{-2em} {\bf Senior Autonomy Engineer} \hfill June, 2017 to \emph{now}
\item Conducting research and development for autonomous driving.
\item Designing and owning the production deep learning model for predicting the trajectories of the other vehicle actors on the road. The model predicts multiple trajectories for each actor with probabilities, uncertainty estimation, and vehicle kinematics constraints.
\item The works are patented and published in \emph{ICRA 2019} and \emph{NeurIPS 2018 MLITS Workshop}.
\end{rSubsection}
\end{rSection}
\vspace{-.15in}
...
\item Proposed five parameter server optimizations exploiting the collected info, achieving up to 50$\times$ speedup.
% }
\end{rSubsection}
\vspace{-.1in}
...
\vspace{-.1in}
\hspace{-1em} \textbf{Program Committee}: \emph{ACM Symposium on Cloud Computing} \hfill {2017 and 2018}
