...
2220A, Collaborative Inovation Center,       & (651)403-3366 \\
5000 Forbes Ave, Pittsburgh, PA, 15213       & \email{hengganc@ece.cmu.edu}\\
\end{tabular}
...
{\bf Software systems for big data analytics}
...
\item[] Ph.D.,
             {Parallel Data Lab, Electrical and Computer Engineering},
             \emph{Expected:} 2018
        \begin{innerlist}
...
\item[] B.S.,
        ...
        \begin{innerlist}
        \item \emph{Outstanding Graduate Award}
        \item \emph{Outstanding Diploma Thesis Award}
        \end{innerlist}
...
\begin{innerlist}
\item[] {\bf LazyTable}:
    A parameter server library for parallel Machine Learning.
    LazyTable manages the model parameters for data parallel ML applications,
    where a collection of ML workers concurrently iterate on
    partitioned input data and make adjustments to the shared model parameters.
    We implemented several representative ML applications on top of LazyTable,
    including movie recommendation (Matrix Factorization using SGD),
    topic modeling (LDA using Gibbs Sampling),
    image classification (Multi-class Logistic Regression using SGD), and PageRank.
\vspace{.1in}
\item[] {\bf Stale Synchronous Parallel (SSP)}:
    A model for synchronizing the progress of parallel ML workers.
    SSP is a middle ground between the BSP approach (where the workers wait at barriers)
    and the Async approach (where the workers never wait).
    SSP allows each worker to be a bounded number of iterations ahead of the slowest one,
    thus making data staleness a tunable parameter and
    allowing us to explicitly trade data freshness for speed.
    Our experiments show that SSP helps reduce the convergence time a lot for our ML benchmarks. \\
    \emph{Papers published in NIPS'13 and ATC'14. SSP is supported in LazyTable.}
\vspace{.1in}
\item[] {\bf Parameter server for iterative ML}:
    Many iterative ML algorithms, including PageRank, SGD, and Gibbs Sampling,
    have the property that the same (or nearly the same) sequence of accesses
    is applied to the parameter server every iteration.
    This repeated sequence can be exploited to improve the performance of parameter servers.
    We implemented and evaluated two methods to collect
    this repeating access sequence from the application,
    as well as five specializations to speed up parameter servers using the collected information.
    Our experiments show that these optimizations greatly reduce the total run time of our application benchmarks by up to 98\%. \\
    \emph{Paper published in SoCC'14. The ideas are implemented in LazyTable.}
\end{innerlist}
...
\item[] {\bf Ingest-time transformation for big time series data}:
    ...
    \emph{Paper to be presented in SoCC'15}.
\end{innerlist}

\halfblankline

\textbf{Research Assistant} \hfill {May 2010 to Jul 2012}\\
Cloud Computing Lab, Tsinghua University\\
Advisor: Dan Li
\begin{innerlist}
\item[] {\bf Multi-class Bloom Filter (MBF)}:
    We extended the concept of standard bloom filters to MBF,
    where different elements are assigned different numbers of hashes
    in the BF array, based on their prior probabilities,
    reducing the false positive rate.
    In this work, we also prove the feasibility of using MBF to
    implement scalable multicast packet switching in data centers networks.\\
    \emph{Paper published in ICNP'11}.
\end{innerlist}

\halfblankline

\textbf{Research Assistant (Internship)} \hfill {Jun 2011 to Aug 2011}\\
Photonics Networking Research Laboratory, Stanford University\\
Advisors: Moises Ribeiro and Leonid Kazovsky
\begin{innerlist}
\item[] {\bf Optical Cross-braced Hypercube (OCH)}:
    A traffic-adaptive data center architecture that adds simple 2x2 optical switches
    to a traditional hypercube topology.
    This architecture can adaptively give short-cuts to the heavy traffic flows
    and thus reduce their impact on the intermediary servers as well as the total traffic in the network. \\
    \emph{Paper published in OFC'12}.
\end{innerlist}
...
\begin{innerlist}
\item[] Outstanding Graduate, Tsinghua University  \hfill Jul 2012
\item[] Outstanding Graduate, Beijing, China \hfill Jul 2012
\item[] Outstanding Diploma Thesis, Tsinghua University \hfill Jul 2012
\item[] National Scholarship, China \hfill Nov 2011
\item[] National Scholarship, China \hfill Nov 2010
\end{innerlist}

\section{Courses}

\textbf{Carnegie Mellon University (GPA 3.91/4)}

15712 Advanced Topics in Operating Systems, Fall 2014. Course project:
    \begin{innerlist}
    \item[] Straggler Mitigation in Parallel ML
    \end{innerlist}
\vspace{.1in}

10780 Graduate Artificial Intelligence, Spring 2014. Course project:
    \begin{innerlist}
    \item[] An online method adaptively tuning data staleness bound under SSP model
    \end{innerlist}
\vspace{.1in}
...
18746/15746 Storage Systems  \hfill Spring 2015
\vspace{.1in}
...
\begin{innerlist}
    \item[] C, C$+$$+$, Python, Java, Matlab, Perl, Scala
\end{innerlist}
\vspace{.1in}

\textbf{Software Systems}
\begin{innerlist}
    \item[] git, LaTeX, svn, GraphLab, HBase, OpenTSDB, Spark, Hadoop
\end{innerlist}
\vspace{.1in}
